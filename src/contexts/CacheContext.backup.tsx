...
export function CacheProvider({ children }: { children: ReactNode }) {
  ...
  // Salvar cache no localStorage apenas quando necessário (desabilitado temporariamente para evitar loops)
  useEffect(() => {
  ...
  useEffect(() => {
    if (user && !loading && !isInitialized) {
      ...
      // Marcar como inicializado ANTES de qualquer carregamento para evitar loops
  ...
  }, [user?.id, loading, isInitialized]) // Simplificar dependências para evitar loops

  // Debug: log do estado do cache para diagnóstico (apenas quando necessário)
  useEffect(() => {
  ...
  }, [user?.id, isInitialized]) // Simplificar dependências
