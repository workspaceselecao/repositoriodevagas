...
async function popular() {
  try {
    ...
    console.log(`✅ Lidas ${vagas.length} vagas`)
    ...
    console.log('📝 Inserindo vagas em lotes de 5...')
    ...
    for (let i = 0; i < vagasToInsert.length; i += batchSize) {
      ...
      if (error) {
        console.log(`❌ Lote ${lote}/${totalLotes} falhou:`, error.message)
      } else {
        const inseridas = data?.length || 0
        total += inseridas
        console.log(`✅ Lote ${lote}/${totalLotes}: ${inseridas} vagas inseridas`)
      }
    }
    ...
    if (stats) {
      const clientes = [...new Set(stats.map((v: any) => v.cliente))].length
      const sites = [...new Set(stats.map((v: any) => v.site))].length
      const categorias = [...new Set(stats.map((v: any) => v.categoria))].length
